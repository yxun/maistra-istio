...
func (c *Controller) InstancesByPort(svc *model.Service, servicePort int) []*model.ServiceInstance {
	var instances []*model.ServiceInstance
	c.storeLock.RLock()
	defer c.storeLock.RUnlock()
	for _, instance := range c.instanceStore[svc.Hostname] {
		if instance.ServicePort.Port == servicePort {
...
func (c *Controller) PodInformation(_ sets.Set[types.NamespacedName]) ([]*model.WorkloadInfo, []string) {
	// Currently, Ambient is not supported
	return []*model.WorkloadInfo{}, []string{}
}

//nolint:lll
func (c *Controller) AdditionalPodSubscriptions(_ *model.Proxy, _ sets.Set[string], _ sets.Set[string]) sets.Set[string] {
	// Currently, Ambient is not supported
	return sets.New[string]()
}

func (c *Controller) Policies(_ sets.Set[model.ConfigKey]) []*security.Authorization {
	// Currently, Ambient is not supported
...
func (c *Controller) AddressInformation(addresses sets.String) ([]*model.AddressInfo, sets.String) {
	// Currently, Ambient is not supported
	return []*model.AddressInfo{}, nil
}

func (c *Controller) Waypoint(_ model.WaypointScope) []netip.Addr {
	// Currently, Ambient is not supported
	return []netip.Addr{}
}

func (c *Controller) WorkloadsForWaypoint(_ model.WaypointScope) []*model.WorkloadInfo {
	// Currently, Ambient is not supported
...
func (c *Controller) createService(opts createServiceOptions) (*model.Service, []*model.ServiceInstance) {
	...
	svc := &model.Service{
		Attributes: model.ServiceAttributes{
			...
			ExportTo:        sets.New[visibility.Instance](opts.serviceVisibility),
...
func (c *Controller) getEgressServiceAddrs() ([]model.NetworkGateway, []string) {
	endpointSlices := common.EndpointSlicesForService(c.rm.EndpointSliceInformer(), c.egressName, c.namespace)
	if len(endpointSlices) == 0 {
		c.logger.Errorf("failed to retrieve EndpointSlices for federation egress gateway %s: %s", c.egressName)
		return nil, nil
	}
	serviceAccountByIP := map[string]string{}
	if !c.useDirectCalls {
		serviceAccountByIP = common.ServiceAccountsForService(c.rm.ServiceInformer(), c.rm.PodInformer(), c.egressName, c.namespace)
		if serviceAccountByIP == nil {
			c.logger.Warnf("failed to retrieve ServiceAccount information for imported services accessed through %s",
				c.egressName)
		}
	}
	var addrs []model.NetworkGateway
	uniqueSAs := sets.New[string]()
	...
	return addrs, sets.SortedList(uniqueSAs)
}
...
func (c *Controller) GetProxyServiceTargets(node *model.Proxy) []model.ServiceTarget {
	return make([]model.ServiceTarget, 0)
}
...
func (c *Controller) AppendServiceHandler(_ model.ServiceHandler) {
	// TODO
}
