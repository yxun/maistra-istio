...
func (c *Controller) getRootCertForMesh(instance *v1.ServiceMeshPeer) (string, error) {
	...
	case "", "ConfigMap":
		cm := c.rm.ConfigMapInformer().Get(name, instance.Namespace)
		if cm == nil {
			return "", fmt.Errorf("error getting configmap %s in namespace %s", name, instance.Namespace)
		}
...
func (c *Controller) updateRootCert(trustDomain string, rootCert string) {
	...
	c.xds.ConfigUpdate(&model.PushRequest{
		Full:   true,
		Reason: model.NewReasonStats([]model.TriggerReason{model.GlobalUpdate}...),
	})
}
