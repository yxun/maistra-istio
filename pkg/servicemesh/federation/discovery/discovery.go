...
func init() {
	...
	schemasBuilder.MustAdd(collections.DestinationRule)
	schemasBuilder.MustAdd(collections.VirtualService)
	schemasBuilder.MustAdd(collections.Gateway)
	schemasBuilder.MustAdd(collections.AuthorizationPolicy)
	// XXX: we should consider adding this directly to the service registry
	schemasBuilder.MustAdd(collections.ServiceEntry)
...
func (c *Controller) deleteDiscoveryResources(
	...
	if err := c.Delete(collections.ServiceEntry.GroupVersionKind(),
	...
	if err := c.Delete(collections.VirtualService.GroupVersionKind(),
	...
	if err := c.Delete(collections.Gateway.GroupVersionKind(),
	...
	if err := c.Delete(collections.DestinationRule.GroupVersionKind(),
		egressName, instance.Namespace, nil); err != nil && !errors.Is(err, memoryStoreErrNotFound) {
		c.Logger.Errorf("error deleting discovery DestinationRule %s for Federation cluster %s: %s",
			egressName, instance.Name, err)
		allErrors = append(allErrors, err)
	}
	if err := c.Delete(collections.AuthorizationPolicy.GroupVersionKind(),
		rootName, instance.Namespace, nil); err != nil && !errors.Is(memoryStoreErrNotFound, err) {
...
func (c *Controller) createDiscoveryResources(
	...
	defer func() {
		if err != nil {
			...
			if s != nil {
				if newErr := c.Delete(collections.ServiceEntry.GroupVersionKind(),
			...
			if ap != nil {
				if newErr := c.Delete(collections.AuthorizationPolicy.GroupVersionKind(),
			...
			if dr != nil {
				...
				if newErr := c.Delete(collections.DestinationRule.GroupVersionKind(),
			...
			if ig != nil {
				...
				if newErr := c.Delete(collections.Gateway.GroupVersionKind(),
			...
			if evs != nil {
				...
				if newErr := c.Delete(collections.VirtualService.GroupVersionKind(),
...
func (c *Controller) discoveryService(instance *v1.ServiceMeshPeer) *config.Config {
	...
	serviceSpec := &rawnetworking.ServiceEntry{
		...
		Ports: []*rawnetworking.ServicePort{
			{
	...
	service := &config.Config{
		Meta: config.Meta{
			GroupVersionKind: collections.ServiceEntry.GroupVersionKind(),
			Name:             name,
...
func (c *Controller) discoveryDestinationRule(instance *v1.ServiceMeshPeer) *config.Config {
	...
	dr := &config.Config{
		Meta: config.Meta{
			GroupVersionKind: collections.DestinationRule.GroupVersionKind(),
			Name:             name,
			Namespace:        instance.Namespace,
			Annotations: map[string]string{
				generationAnnotation: fmt.Sprintf("%d", instance.Generation),
