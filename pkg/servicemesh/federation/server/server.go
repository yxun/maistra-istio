...
func NewServer(opt Options) (*Server, error) {
	...
	router.HandleFunc("/v1/services/{mesh}", fed.handleServiceList)
	router.HandleFunc("/v1/watch/{mesh}", fed.handleWatch)
	fed.httpServer.Handler = router
	return fed, nil
}
...
func (s *Server) UpdateService(prev, curr *model.Service, event model.Event) {
	s.meshes.Range(func(_, value any) bool {
		value.(*meshServer).serviceUpdated(curr, event)
		return true
	})
}
...
func (s *meshServer) getServiceMessage(svc *model.Service, exportedName *federationmodel.ServiceKey) *federationmodel.ServiceMessage {
	...
	for _, port := range svc.Ports {
		...
		if addServiceSAs {
			for _, si := range s.env.InstancesByPort(svc, port.Port) {
...
func (s *meshServer) checkServiceExportTo(svc *model.Service) bool {
	...
	if svc.Attributes.ExportTo.Contains(visibility.Public) {
		return true
	}
	if svc.Attributes.ExportTo.Contains(visibility.Private) && s.mesh.Namespace == svc.Attributes.Namespace {
		return true
	}
	if svc.Attributes.ExportTo.Contains(visibility.Instance(s.mesh.Namespace)) {
		return true
	}
...
func (s *meshServer) updateGatewayServiceAccounts() bool {
	...
	for _, si := range s.env.InstancesByPort(gatewayService, common.DefaultFederationPort) {
