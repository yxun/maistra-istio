...
func (s *meshServer) deleteExportResources(source federationmodel.ServiceKey, target *federationmodel.ServiceMessage) error {
	...
	_ = s.configStore.Delete(collections.Gateway.GroupVersionKind(), resourceName, s.mesh.Namespace, nil)
	_ = s.configStore.Delete(collections.VirtualService.GroupVersionKind(), resourceName, s.mesh.Namespace, nil)
...
func (s *meshServer) removeServiceFromAuthorizationPolicy(target *federationmodel.ServiceMessage) error {
	...
	rawAP := s.configStore.Get(collections.AuthorizationPolicy.GroupVersionKind(), name, s.mesh.Namespace)
	if rawAP == nil {
		// nothing to remove
...
func (s *meshServer) createExportResources(source federationmodel.ServiceKey, target *federationmodel.ServiceMessage) error {
	...
	if rawGateway := s.configStore.Get(
		collections.Gateway.GroupVersionKind(), gateway.Name, s.mesh.Namespace); rawGateway == nil {
	...
	if rawVS := s.configStore.Get(collections.VirtualService.GroupVersionKind(), vs.Name, s.mesh.Namespace); rawVS == nil {
