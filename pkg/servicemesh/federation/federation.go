...
func init() {
	...
	discovery.Schemas.ForEach(func(s resource.Schema) (done bool) {
		// only error is already exists, which we don't care about
		_ = schemasBuilder.Add(s)
		return false
	})
	server.Schemas.ForEach(func(s resource.Schema) (done bool) {
		// only error is already exists, which we don't care about
		_ = schemasBuilder.Add(s)
		return false
	})
...
func internalNew(opt Options, cs maistraclient.Interface) (*Federation, error) {
	...
	}, opt.KubeClient.GetMemberRollController())
	if err != nil {
		return nil, err
	}
