...
func EndpointSlicesForService(
	endpointSliceLister kclient.Client[*discoveryv1.EndpointSlice], name, namespace string,
) []*discoveryv1.EndpointSlice {
	return endpointSliceLister.List(namespace, kubelabels.Set{discoveryv1.LabelServiceName: name}.AsSelector())
}
...
func ServiceAccountsForService(
	serviceLister kclient.Client[*corev1.Service], podLister kclient.Client[*corev1.Pod], name, namespace string,
) map[string]string {
	service := serviceLister.Get(name, namespace)
	if service == nil {
		return nil
	}

	pods := podLister.List(namespace, kubelabels.Set(service.Spec.Selector).AsSelector())
	if len(pods) == 0 {
		return nil
	}
